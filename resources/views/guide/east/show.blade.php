...
<div class="clan-overview grid grid-cols-1 md:grid-cols-[240px_minmax(0,1fr)] gap-6">
    ...
    <div class="welcome-section">
        <div class="welcome-content">
            <a href="{{ route('guide.east.index') }}" class="text-white/70 hover:text-white block mb-4">← Назад к списку</a>
                    ...
                    @auth
                        @if(auth()->id() === $guide->user_id || auth()->user()->role === 'admin')
                            <div class="mt-4 flex gap-3">
                                <a href="{{ route('guide.east.edit', $guide) }}" class="welcome-title text-white hover:text-white/80 hover:underline cursor-pointer transition-colors" style="font-size: 1rem; line-height: 1.5;">Редактировать</a>
                                <form action="{{ route('guide.east.destroy', $guide) }}" method="POST" onsubmit="return confirm('Удалить запись #{{ $guide->id }}?');">
                                    @csrf
                                    @method('DELETE')
