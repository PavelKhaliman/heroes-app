...
@section('title', 'Клан HEROES - Добавить клан в кос лист')

@section('content')
<div class="clan-overview">
    <div class="welcome-section text-center">
        <div class="welcome-content">
			<h1 class="welcome-title mb-8 text-white">Добавить клан</h1>
            <form action="{{ route('admin.clan.coslist.store.guild') }}" method="POST" class="max-w-2xl mx-auto text-left">
                @csrf
				...
				<div class="mb-4">
					<label for="master" class="block text-left mb-2 text-white">Мастер</label>
                    <input id="master" name="master" type="text" value="{{ old('master') }}" class="w-full p-3 rounded-lg bg-white/10 border border-white/20 text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-orange-500" placeholder="Введите ник мастера">
                    @error('master')
                        <p class="mt-2 text-red-400 text-sm">{{ $message }}</p>
                    @enderror
                </div>
