...
@section('title', 'Галерея - Скриншот')

@section('content')
<div class="clan-overview">
    <div class="welcome-section">
        <div class="welcome-content">
            <a href="{{ route('gallery.screenshot.index') }}" class="text-white/70 hover:text-white block mb-4">← Назад к списку</a>
                    ...
                    @auth
                        @if(auth()->id() === $photo->user_id || auth()->user()->role === 'admin')
                            <div class="mt-4 flex gap-3">
                                <a href="{{ route('gallery.screenshot.edit', $photo) }}" class="welcome-title text-white hover:text-white/80 hover:underline cursor-pointer transition-colors" style="font-size: 1rem; line-height: 1.5;">Редактировать</a>
                                <form action="{{ route('gallery.screenshot.destroy', $photo) }}" method="POST" onsubmit="return confirm('Удалить запись #{{ $photo->id }}?');">
                                    @csrf
                                    @method('DELETE')
