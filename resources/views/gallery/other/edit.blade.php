...
@section('title', 'Галерея - Редактировать запись')

@section('content')
<div class="clan-overview">
    <div class="welcome-section">
        <div class="welcome-content">
            <h1 class="welcome-title mb-6">Редактировать запись #{{ $photo->id }}</h1>

            <form action="{{ route('gallery.other.update', $photo) }}" method="POST" enctype="multipart/form-data" class="max-w-2xl text-left">
                @csrf
                @method('PUT')
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('gallery.other.show', $photo) }}" class="welcome-title text-white hover:text-white/80 transition-colors" style="font-size: 1rem; line-height: 1.5;">Отмена</a>
                </div>
            </form>
        </div>
    </div>
