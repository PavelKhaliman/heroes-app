...
@section('title', 'Галерея - Добавить другое')

@section('content')
<div class="clan-overview">
    <div class="welcome-section">
        <div class="welcome-content">
            <h1 class="welcome-title mb-6">Добавить запись</h1>

            <form action="{{ route('gallery.other.store') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl text-left" onsubmit="const b=this.querySelector('[data-submit-btn]'); if(b){b.disabled=true; b.classList.add('opacity-50','pointer-events-none'); b.textContent='Загрузка...';}">
                @csrf
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('gallery.other.index') }}" class="welcome-title text-white hover:text-white/80 transition-colors" style="font-size: 1rem; line-height: 1.5;">Отмена</a>
                </div>
            </form>
        </div>
    </div>
