...
@section('title', 'Галерея - Фотографии')
    ...
    <div class="welcome-section">
        <div class="welcome-content">
            <div class="flex items-center justify-between mb-6">
                <h1 class="welcome-title">Фотографии</h1>
                <a href="{{ route('gallery.photo.create') }}" class="inline-block welcome-title text-white hover:text-white/80 transition-colors" style="font-size: 1rem; line-height: 1.5;">Добавить фото</a>
            </div>

            @if($photos->isEmpty())
                <div class="text-white/80">Фотографий пока нет.</div>
            @else
                    ...
                    @foreach($photos as $photo)
                        <a href="{{ route('gallery.photo.show', $photo) }}" class="block group bg-white/5 rounded-lg overflow-hidden hover:bg-white/10 transition-colors">
                            <div class="aspect-video bg-black/30 overflow-hidden">
                                <img src="{{ route('media', ['path' => $photo->image_path]) }}" alt="Фото #{{ $photo->id }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform" />
                            </div>
