...
<div class="clan-overview">
    <div class="welcome-section">
        <div class="welcome-content">
            <a href="{{ route('gallery.photo.index') }}" class="text-white/70 hover:text-white block mb-4">← Назад к списку</a>
            <h1 class="text-white text-2xl md:text-3xl mb-4">{{ $photo->title }}</h1>

            <div class="bg-white/5 rounded-lg overflow-hidden">
                <div class="bg-black/30">
                    <img src="{{ route('media', ['path' => $photo->image_path]) }}" alt="Фото #{{ $photo->id }}" class="w-full object-contain" />
                </div>
                    ...
                    @auth
                        @if(auth()->id() === $photo->user_id || auth()->user()->role === 'admin')
                            <div class="mt-4 flex gap-3">
                                <a href="{{ route('gallery.photo.edit', $photo) }}" class="welcome-title text-white hover:text-white/80 hover:underline cursor-pointer transition-colors" style="font-size: 1rem; line-height: 1.5;">Редактировать</a>
                                <form action="{{ route('gallery.photo.destroy', $photo) }}" method="POST" onsubmit="return confirm('Удалить фото #{{ $photo->id }}?');">
                                    @csrf
                                    @method('DELETE')
